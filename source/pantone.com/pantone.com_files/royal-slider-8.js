...
(function ($) {
    ...
    function RoyalSlider(element, options) {
        ...
        if (this.settings.controlNavEnabled) {
            ...
            if (self.settings.controlNavMenu) {
                _navigation = $('nav.subCategoryNav ul');
                this.navItems = _navigation.find('li a');
    ...
    RoyalSlider.prototype = {
        ...
        goTo: function (id, silent, fromNav, fromTouch, customSpeed) {
            ...
            if (!this.isAnimating) {
                ...
                if (this.settings.controlNavMenu) {
                    if ($('#productCategRotator .royalSlide.current').size() > 0)
                        $('#productCategRotator .royalSlide.current').removeClass('current');
                    this.slidesArr[id].slide.addClass('current');
                }
        ...
        _resizeContainerDiv: function (id, firstLoadedCallback) {
            if (this.settings.controlNavMenu) {
                var self = this;
                height = this.slidesArr[id].height;
                this.slidesArr[id].animate("height", height);
            }
        },
        _updateControlNav: function (fromNav) {
            if (this.settings.controlNavEnabled) {
                this.navItems.eq(this.lastSlideId).removeClass('current coy').parent().removeClass('current');

                this.navItems.eq(this.currentSlideId).addClass("current coy").parent().addClass('current');
        ...
        _startSlider: function () {
            ...
            if (this.settings.controlNavMenu) {
                if ($('#productCategRotator .royalSlide.current').size() > 0)
                    $('#productCategRotator .royalSlide.current').removeClass('current').parent().removeClass('current');
                this.slidesArr[self.currentSlideId].slide.addClass('current').parent().addClass('current');
            }
        ...
        _onNavItemClick: function (e) {
            if (this.settings.controlNavMenu) {
                this.goTo($(e.currentTarget.parentElement.parentElement).children(e.currentTarget.parentElement.nodeName).index($(e.currentTarget.parentElement)), false, true);
            }
            else {
    ...
    $.fn.royalSlider.defaults = {
        ...
        controlNavMenu: false,                       //Use Category Menu PTN 03/28/13
