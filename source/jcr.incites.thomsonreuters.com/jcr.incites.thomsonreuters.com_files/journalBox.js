		...
		function box(g) {
			g
					.each(function(d, i) {
						...
						// Add a line for journals impact factor
						var journalLine = g.selectAll("line").data(
								[ currentJournalIf ]);

						journalLine.enter().append("line").attr("x1", -30)
								.attr("y1", x1).attr("x2", width * 2).attr(
										"y2", x1).style("stroke", "#000")
								.style("stroke-width", "1.5px").style("fill",
										"#fff");
						...
						whisker.exit().transition().duration(duration).attr(
								"y1", x1).attr("y2", x1).style("opacity", 1)
								.remove();
						...
						g.append('text').text(categoryNames[i]).attr(
								'transform', "translate(-10, 237)");
